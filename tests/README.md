...
* tests Python 2 runtime and development packages are installed, using non-system package sources, where available
...
1. `test-basic`
    ...
    * tests PIP package manager is installed
2. `test-system-only`
    * tests Python 2 runtime and development packages are installed, using system-only package sources
    * tests PIP package manager is installed
...
to bring up a local testing environment
2. `$ cd python2/tests/provisioning/site-testing-local`
3. `$ vagrant up`
4. `$ cd ..`
...
1. `$ cd python2/tests/provisioning`
...
Results for these tests can be found [here](https://semaphoreci.com/bas-ansible-roles-collection/python2).
